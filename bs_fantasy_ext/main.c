...
int AnopeInit(void) {
	...
	Command *c = NULL;
	c = createCommand("IRCOPS", list_global_opers, NULL, -1, -1, -1, -1, -1);
...
int do_fantasy(int ac, char **av) {
	...
	if (stricmp(av[0], "topic") == 0) {
		if (ac == 3)
			anope_cmd_notice(ci->bi->nick, u->nick, "Channel Topic: %s", (((c->topic))?c->topic:""));
	...
	if (stricmp(av[0], "showtopic") == 0) {
		if (ac == 3) {
			anope_cmd_privmsg(ci->bi->nick, ci->name, "Channel Topic: %s", (((c->topic))?c->topic:(((ci->last_topic))?ci->last_topic:"")));
		}
		/*
		else if (ac == 4) {
			ChannelInfo *stci = NULL;
			Channel *stc = NULL;
			char *chan;
			char *tmp;

			chan = myStrGetToken(av[3], ' ', 0);
			tmp = myStrGetToken(av[3], ' ', 1);

			if (chan && !tmp) {
				if (!(stci = cs_findchan(chan))) {
					anope_cmd_notice(ci->bi->nick, u->nick, "Channel %s not found", chan);
				}
				else if (!(stc = findchan(stci->name))) {
					anope_cmd_notice(ci->bi->nick, u->nick, "Channel %s not found", stci->name);
				}
				if ((stc->mode & CMODE_s) || (stc->mode & CMODE_p)) {
					anope_cmd_notice2(ci->bi->nick, u->nick, "This command does not work with secret/private channels");
				}
				else {
					anope_cmd_privmsg(ci->bi->nick, ci->name, "%s Channel Topic: %s", stci->name, (((stc->topic))?stc->topic:(((stci->last_topic))?stci->last_topic:"")));
				}
			}
...
int do_fantasy_denied(int ac, char **av) {
	...
	if (stricmp(av[0], "topic") == 0) {
		if (ac == 3)
			anope_cmd_notice(ci->bi->nick, u->nick, "Channel Topic: %s", c->topic);
		else
			notice_lang(ci->bi->nick, u, ACCESS_DENIED);
	} else
#endif
